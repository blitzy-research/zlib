...
1. **[High]** Conduct formal human code review & sign-off of the migration (80,352 lines across the 40 `src/` modules today; AAP baseline 32,354 LOC), focusing on the `unsafe` FFI boundary and its `// SAFETY:` justifications (~16h). This is the one item that cannot be automated away.
...
|---|---:|---|---|---|
| Human code review & sign-off of the safety-critical migration (80,352 lines today; AAP baseline 32,354 LOC) | 16 | High | — | Outstanding; cannot be automated away |
...
**Measured distribution.** Counting `\bunsafe\b` construct lines with whole-line comments discarded, the same method `SECURITY.md` publishes beside its own table: **1,667** in `src/ffi/**` (`inflate.rs` 538, `deflate.rs` 373, `gz.rs` 212, `types.rs` 193, `util.rs` 184, `mod.rs` 113, `alloc.rs` 54); **51** in `src/lib.rs`, of which 22 are the private no-`std` runtime support and 29 the in-crate boundary tests that police it. Note that `doc/technical-specifications.md` §0.6.2 additionally strips *trailing* comments and therefore reports **49** for `src/lib.rs`, split **22 / 27**; the two-line gap is the string-literal fixtures `"// unsafe\n"` and `"//! unsafe\n"` that the boundary test feeds its own classifier, and the `src/ffi/**` total is 1,667 under either scan. Also **2 in `src/stream.rs`, and those are type aliases only** — `ZallocFn` and `ZfreeFn` merely *name* the C hook signatures the crate must interoperate with, and the file contains no executable `unsafe` block at all; and **zero** across all eight core module groups. There are **597** `// SAFETY:` comments in `src/`. The AAP's recorded baseline of 568 / 20 / 284 predates the hardening work and is retained here only as a historical datum; counting `unsafe { … }` *blocks* instead answers a different question and yields 1,270 which is the figure `doc/project-guide.md` §8.5 quotes.
...
|---|---|---|---|---|---|
| No formal human code review or sign-off across the migration surface (80,352 lines today; AAP baseline 32,354 LOC) | Governance | High | High | Human review of the `unsafe` FFI boundary and its `// SAFETY:` justifications; cannot be automated away | Open |
